...
//
// Table class for frl_zona
//
class cfrl_zona extends cTable {
	var $frl_zon_id;
	var $zona;
	var $cod_provincia;
	var $cod_distrito;
	...
	function __construct() {
		...
		$this->TableVar = 'frl_zona';
		$this->TableName = 'frl_zona';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`frl_zona`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// frl_zon_id
		$this->frl_zon_id = new cField('frl_zona', 'frl_zona', 'x_frl_zon_id', 'frl_zon_id', '`frl_zon_id`', '`frl_zon_id`', 3, -1, FALSE, '`frl_zon_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->frl_zon_id->Sortable = TRUE; // Allow sort
		$this->frl_zon_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['frl_zon_id'] = &$this->frl_zon_id;

		// zona
		$this->zona = new cField('frl_zona', 'frl_zona', 'x_zona', 'zona', '`zona`', '`zona`', 200, -1, FALSE, '`zona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->zona->Sortable = TRUE; // Allow sort
		$this->fields['zona'] = &$this->zona;

		// cod_provincia
		$this->cod_provincia = new cField('frl_zona', 'frl_zona', 'x_cod_provincia', 'cod_provincia', '`cod_provincia`', '`cod_provincia`', 3, -1, FALSE, '`cod_provincia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cod_distrito
		$this->cod_distrito = new cField('frl_zona', 'frl_zona', 'x_cod_distrito', 'cod_distrito', '`cod_distrito`', '`cod_distrito`', 3, -1, FALSE, '`cod_distrito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// populacao
		$this->populacao = new cField('frl_zona', 'frl_zona', 'x_populacao', 'populacao', '`populacao`', '`populacao`', 3, -1, FALSE, '`populacao`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// eleitores
		$this->eleitores = new cField('frl_zona', 'frl_zona', 'x_eleitores', 'eleitores', '`eleitores`', '`eleitores`', 3, -1, FALSE, '`eleitores`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`frl_zona`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->frl_zon_id->setDbValue($conn->Insert_ID());
			$rs['frl_zon_id'] = $this->frl_zon_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('frl_zon_id', $rs))
				ew_AddFilter($where, ew_QuotedName('frl_zon_id', $this->DBID) . '=' . ew_QuotedValue($rs['frl_zon_id'], $this->frl_zon_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`frl_zon_id` = @frl_zon_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->frl_zon_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@frl_zon_id@", ew_AdjustSql($this->frl_zon_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("frl_zonaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("frl_zonaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "frl_zonaadd.php?" . $this->UrlParm($parm);
		else
			$url = "frl_zonaadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("frl_zonaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("frl_zonaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("frl_zonadelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "frl_zon_id:" . ew_VarToJson($this->frl_zon_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->frl_zon_id->CurrentValue)) {
			$sUrl .= "frl_zon_id=" . urlencode($this->frl_zon_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["frl_zon_id"]))
				$arKeys[] = ew_StripSlashes($_POST["frl_zon_id"]);
			elseif (isset($_GET["frl_zon_id"]))
				$arKeys[] = ew_StripSlashes($_GET["frl_zon_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->frl_zon_id->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// frl_zon_id
		$this->frl_zon_id->EditAttrs["class"] = "form-control";
		$this->frl_zon_id->EditCustomAttributes = "";
		$this->frl_zon_id->EditValue = $this->frl_zon_id->CurrentValue;
		$this->frl_zon_id->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->frl_zon_id->Exportable) $Doc->ExportCaption($this->frl_zon_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->frl_zon_id->Exportable) $Doc->ExportField($this->frl_zon_id);
