...
$frl_circulo_edit = NULL; // Initialize page object first

class cfrl_circulo_edit extends cfrl_circulo {
	...
	// Page object name
	var $PageObjName = 'frl_circulo_edit';
	...
	function Page_Main() {
		...
		if (@$_GET["frl_cir_id"] <> "") {
			$this->frl_cir_id->setQueryStringValue($_GET["frl_cir_id"]);
		}
		...
		// Check if valid key
		if ($this->frl_cir_id->CurrentValue == "") {
			$this->Page_Terminate("frl_circulolist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("frl_circulolist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "frl_circulolist.php")
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->frl_cir_id->FldIsDetailKey)
			$this->frl_cir_id->setFormValue($objForm->GetValue("x_frl_cir_id"));
		if (!$this->circulo->FldIsDetailKey) {
			$this->circulo->setFormValue($objForm->GetValue("x_circulo"));
		}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->frl_cir_id->CurrentValue = $this->frl_cir_id->FormValue;
		$this->circulo->CurrentValue = $this->circulo->FormValue;
	...
	function RenderRow() {
		...
		$this->cod_zona->ViewCustomAttributes = "";
			...
			$this->circulo->EditValue = ew_HtmlEncode($this->circulo->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->circulo->FldIsDetailKey && !is_null($this->circulo->FormValue) && $this->circulo->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->circulo->FldCaption(), $this->circulo->ReqErrMsg));
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// circulo
			$this->circulo->SetDbValueDef($rsnew, $this->circulo->CurrentValue, "", $this->circulo->ReadOnly);
...
// Create page object
if (!isset($frl_circulo_edit)) $frl_circulo_edit = new cfrl_circulo_edit();

// Page init
$frl_circulo_edit->Page_Init();

// Page main
$frl_circulo_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$frl_circulo_edit->Page_Render();
?>
...
var CurrentForm = ffrl_circuloedit = new ew_Form("ffrl_circuloedit", "edit");

// Validate form
ffrl_circuloedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_circulo");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $frl_circulo->circulo->FldCaption(), $frl_circulo->circulo->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_cod_provincia");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $frl_circulo->cod_provincia->FldCaption(), $frl_circulo->cod_provincia->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_cod_distrito");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $frl_circulo->cod_distrito->FldCaption(), $frl_circulo->cod_distrito->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_cod_zona");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $frl_circulo->cod_zona->FldCaption(), $frl_circulo->cod_zona->ReqErrMsg)) ?>");
...
ffrl_circuloedit.Form_CustomValidate = 
...
ffrl_circuloedit.ValidateRequired = true;
<?php } else { ?>
ffrl_circuloedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffrl_circuloedit.Lists["x_cod_provincia"] = {"LinkField":"x_sms_pro_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_provincia","","",""],"ParentFields":[],"ChildFields":["x_cod_distrito"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sms_provincias"};
ffrl_circuloedit.Lists["x_cod_distrito"] = {"LinkField":"x_sms_dis_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_distrito","","",""],"ParentFields":["x_cod_provincia"],"ChildFields":["x_cod_zona"],"FilterFields":["x_cod_provincia"],"Options":[],"Template":"","LinkTable":"sms_distritos"};
ffrl_circuloedit.Lists["x_cod_zona"] = {"LinkField":"x_frl_zon_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_zona","","",""],"ParentFields":["x_cod_distrito"],"ChildFields":[],"FilterFields":["x_cod_distrito"],"Options":[],"Template":"","LinkTable":"frl_zona"};
...
<?php if (!$frl_circulo_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $frl_circulo_edit->ShowPageHeader(); ?>
<?php
$frl_circulo_edit->ShowMessage();
?>
<form name="ffrl_circuloedit" id="ffrl_circuloedit" class="<?php echo $frl_circulo_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($frl_circulo_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $frl_circulo_edit->Token ?>">
<?php } ?>
...
<?php if ($frl_circulo->frl_cir_id->Visible) { // frl_cir_id ?>
	<div id="r_frl_cir_id" class="form-group">
		<label id="elh_frl_circulo_frl_cir_id" class="col-sm-2 control-label ewLabel"><?php echo $frl_circulo->frl_cir_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $frl_circulo->frl_cir_id->CellAttributes() ?>>
<span id="el_frl_circulo_frl_cir_id">
<span<?php echo $frl_circulo->frl_cir_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $frl_circulo->frl_cir_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="frl_circulo" data-field="x_frl_cir_id" name="x_frl_cir_id" id="x_frl_cir_id" value="<?php echo ew_HtmlEncode($frl_circulo->frl_cir_id->CurrentValue) ?>">
<?php echo $frl_circulo->frl_cir_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($frl_circulo->circulo->Visible) { // circulo ?>
	<div id="r_circulo" class="form-group">
		<label id="elh_frl_circulo_circulo" for="x_circulo" class="col-sm-2 control-label ewLabel"><?php echo $frl_circulo->circulo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $frl_circulo->circulo->CellAttributes() ?>>
<span id="el_frl_circulo_circulo">
<input type="text" data-table="frl_circulo" data-field="x_circulo" name="x_circulo" id="x_circulo" size="30" maxlength="40" placeholder="<?php echo ew_HtmlEncode($frl_circulo->circulo->getPlaceHolder()) ?>" value="<?php echo $frl_circulo->circulo->EditValue ?>"<?php echo $frl_circulo->circulo->EditAttributes() ?>>
</span>
<?php echo $frl_circulo->circulo->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($frl_circulo->cod_provincia->Visible) { // cod_provincia ?>
	<div id="r_cod_provincia" class="form-group">
		<label id="elh_frl_circulo_cod_provincia" for="x_cod_provincia" class="col-sm-2 control-label ewLabel"><?php echo $frl_circulo->cod_provincia->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $frl_circulo->cod_provincia->CellAttributes() ?>>
<span id="el_frl_circulo_cod_provincia">
<?php $frl_circulo->cod_provincia->EditAttrs["onchange"] = "ew_UpdateOpt.call(this); " . @$frl_circulo->cod_provincia->EditAttrs["onchange"]; ?>
<select data-table="frl_circulo" data-field="x_cod_provincia" data-value-separator="<?php echo $frl_circulo->cod_provincia->DisplayValueSeparatorAttribute() ?>" id="x_cod_provincia" name="x_cod_provincia"<?php echo $frl_circulo->cod_provincia->EditAttributes() ?>>
<?php echo $frl_circulo->cod_provincia->SelectOptionListHtml("x_cod_provincia") ?>
</select>
<input type="hidden" name="s_x_cod_provincia" id="s_x_cod_provincia" value="<?php echo $frl_circulo->cod_provincia->LookupFilterQuery() ?>">
</span>
<?php echo $frl_circulo->cod_provincia->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($frl_circulo->cod_distrito->Visible) { // cod_distrito ?>
	<div id="r_cod_distrito" class="form-group">
		<label id="elh_frl_circulo_cod_distrito" for="x_cod_distrito" class="col-sm-2 control-label ewLabel"><?php echo $frl_circulo->cod_distrito->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $frl_circulo->cod_distrito->CellAttributes() ?>>
<span id="el_frl_circulo_cod_distrito">
<?php $frl_circulo->cod_distrito->EditAttrs["onchange"] = "ew_UpdateOpt.call(this); " . @$frl_circulo->cod_distrito->EditAttrs["onchange"]; ?>
<select data-table="frl_circulo" data-field="x_cod_distrito" data-value-separator="<?php echo $frl_circulo->cod_distrito->DisplayValueSeparatorAttribute() ?>" id="x_cod_distrito" name="x_cod_distrito"<?php echo $frl_circulo->cod_distrito->EditAttributes() ?>>
<?php echo $frl_circulo->cod_distrito->SelectOptionListHtml("x_cod_distrito") ?>
</select>
<input type="hidden" name="s_x_cod_distrito" id="s_x_cod_distrito" value="<?php echo $frl_circulo->cod_distrito->LookupFilterQuery() ?>">
</span>
<?php echo $frl_circulo->cod_distrito->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($frl_circulo->cod_zona->Visible) { // cod_zona ?>
	<div id="r_cod_zona" class="form-group">
		<label id="elh_frl_circulo_cod_zona" for="x_cod_zona" class="col-sm-2 control-label ewLabel"><?php echo $frl_circulo->cod_zona->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $frl_circulo->cod_zona->CellAttributes() ?>>
<span id="el_frl_circulo_cod_zona">
<select data-table="frl_circulo" data-field="x_cod_zona" data-value-separator="<?php echo $frl_circulo->cod_zona->DisplayValueSeparatorAttribute() ?>" id="x_cod_zona" name="x_cod_zona"<?php echo $frl_circulo->cod_zona->EditAttributes() ?>>
<?php echo $frl_circulo->cod_zona->SelectOptionListHtml("x_cod_zona") ?>
</select>
<input type="hidden" name="s_x_cod_zona" id="s_x_cod_zona" value="<?php echo $frl_circulo->cod_zona->LookupFilterQuery() ?>">
</span>
<?php echo $frl_circulo->cod_zona->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$frl_circulo_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $frl_circulo_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ffrl_circuloedit.Init();
</script>
<?php
$frl_circulo_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$frl_circulo_edit->Page_Terminate();
?>
