...
//
// Table class for sms_distritos
//
class csms_distritos extends cTable {
	var $sms_dis_id;
	var $distrito;
	var $cod_provincia;
	var $populacao;
	...
	function __construct() {
		...
		$this->TableVar = 'sms_distritos';
		$this->TableName = 'sms_distritos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`sms_distritos`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sms_dis_id
		$this->sms_dis_id = new cField('sms_distritos', 'sms_distritos', 'x_sms_dis_id', 'sms_dis_id', '`sms_dis_id`', '`sms_dis_id`', 3, -1, FALSE, '`sms_dis_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sms_dis_id->Sortable = TRUE; // Allow sort
		$this->sms_dis_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sms_dis_id'] = &$this->sms_dis_id;

		// distrito
		$this->distrito = new cField('sms_distritos', 'sms_distritos', 'x_distrito', 'distrito', '`distrito`', '`distrito`', 200, -1, FALSE, '`distrito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->distrito->Sortable = TRUE; // Allow sort
		$this->fields['distrito'] = &$this->distrito;

		// cod_provincia
		$this->cod_provincia = new cField('sms_distritos', 'sms_distritos', 'x_cod_provincia', 'cod_provincia', '`cod_provincia`', '`cod_provincia`', 3, -1, FALSE, '`cod_provincia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// populacao
		$this->populacao = new cField('sms_distritos', 'sms_distritos', 'x_populacao', 'populacao', '`populacao`', '`populacao`', 3, -1, FALSE, '`populacao`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// eleitores
		$this->eleitores = new cField('sms_distritos', 'sms_distritos', 'x_eleitores', 'eleitores', '`eleitores`', '`eleitores`', 3, -1, FALSE, '`eleitores`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`sms_distritos`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->sms_dis_id->setDbValue($conn->Insert_ID());
			$rs['sms_dis_id'] = $this->sms_dis_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('sms_dis_id', $rs))
				ew_AddFilter($where, ew_QuotedName('sms_dis_id', $this->DBID) . '=' . ew_QuotedValue($rs['sms_dis_id'], $this->sms_dis_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`sms_dis_id` = @sms_dis_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->sms_dis_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@sms_dis_id@", ew_AdjustSql($this->sms_dis_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sms_distritosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sms_distritosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sms_distritosadd.php?" . $this->UrlParm($parm);
		else
			$url = "sms_distritosadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sms_distritosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sms_distritosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sms_distritosdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "sms_dis_id:" . ew_VarToJson($this->sms_dis_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->sms_dis_id->CurrentValue)) {
			$sUrl .= "sms_dis_id=" . urlencode($this->sms_dis_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["sms_dis_id"]))
				$arKeys[] = ew_StripSlashes($_POST["sms_dis_id"]);
			elseif (isset($_GET["sms_dis_id"]))
				$arKeys[] = ew_StripSlashes($_GET["sms_dis_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->sms_dis_id->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->sms_dis_id->Exportable) $Doc->ExportCaption($this->sms_dis_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->sms_dis_id->Exportable) $Doc->ExportField($this->sms_dis_id);
