...
$frl_celula_delete = NULL; // Initialize page object first

class cfrl_celula_delete extends cfrl_celula {
	...
	// Page object name
	var $PageObjName = 'frl_celula_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("frl_celulalist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in frl_celula class, frl_celulainfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("frl_celulalist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($frl_celula_delete)) $frl_celula_delete = new cfrl_celula_delete();

// Page init
$frl_celula_delete->Page_Init();

// Page main
$frl_celula_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$frl_celula_delete->Page_Render();
?>
...
var CurrentForm = ffrl_celuladelete = new ew_Form("ffrl_celuladelete", "delete");

// Form_CustomValidate event
ffrl_celuladelete.Form_CustomValidate = 
...
ffrl_celuladelete.ValidateRequired = true;
<?php } else { ?>
ffrl_celuladelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffrl_celuladelete.Lists["x_cod_provincia"] = {"LinkField":"x_sms_pro_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_provincia","","",""],"ParentFields":[],"ChildFields":["x_cod_distrito"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sms_provincias"};
ffrl_celuladelete.Lists["x_cod_distrito"] = {"LinkField":"x_sms_dis_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_distrito","","",""],"ParentFields":[],"ChildFields":["x_cod_zona"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sms_distritos"};
ffrl_celuladelete.Lists["x_cod_zona"] = {"LinkField":"x_frl_zon_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_zona","","",""],"ParentFields":[],"ChildFields":["x_cod_circulo"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"frl_zona"};
ffrl_celuladelete.Lists["x_cod_circulo"] = {"LinkField":"x_frl_cir_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_circulo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"frl_circulo"};
...
<?php $frl_celula_delete->ShowPageHeader(); ?>
<?php
$frl_celula_delete->ShowMessage();
?>
<form name="ffrl_celuladelete" id="ffrl_celuladelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($frl_celula_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $frl_celula_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="frl_celula">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($frl_celula_delete->RecKeys as $key) { ?>
...
<?php if ($frl_celula->frl_cel_id->Visible) { // frl_cel_id ?>
		<th><span id="elh_frl_celula_frl_cel_id" class="frl_celula_frl_cel_id"><?php echo $frl_celula->frl_cel_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($frl_celula->celula->Visible) { // celula ?>
		<th><span id="elh_frl_celula_celula" class="frl_celula_celula"><?php echo $frl_celula->celula->FldCaption() ?></span></th>
<?php } ?>
<?php if ($frl_celula->cod_provincia->Visible) { // cod_provincia ?>
		<th><span id="elh_frl_celula_cod_provincia" class="frl_celula_cod_provincia"><?php echo $frl_celula->cod_provincia->FldCaption() ?></span></th>
<?php } ?>
<?php if ($frl_celula->cod_distrito->Visible) { // cod_distrito ?>
		<th><span id="elh_frl_celula_cod_distrito" class="frl_celula_cod_distrito"><?php echo $frl_celula->cod_distrito->FldCaption() ?></span></th>
<?php } ?>
<?php if ($frl_celula->cod_zona->Visible) { // cod_zona ?>
		<th><span id="elh_frl_celula_cod_zona" class="frl_celula_cod_zona"><?php echo $frl_celula->cod_zona->FldCaption() ?></span></th>
<?php } ?>
<?php if ($frl_celula->cod_circulo->Visible) { // cod_circulo ?>
		<th><span id="elh_frl_celula_cod_circulo" class="frl_celula_cod_circulo"><?php echo $frl_celula->cod_circulo->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$frl_celula_delete->RecCnt = 0;
$i = 0;
while (!$frl_celula_delete->Recordset->EOF) {
	$frl_celula_delete->RecCnt++;
	$frl_celula_delete->RowCnt++;

	// Set row properties
	$frl_celula->ResetAttrs();
	$frl_celula->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$frl_celula_delete->LoadRowValues($frl_celula_delete->Recordset);

	// Render row
	$frl_celula_delete->RenderRow();
?>
	<tr<?php echo $frl_celula->RowAttributes() ?>>
<?php if ($frl_celula->frl_cel_id->Visible) { // frl_cel_id ?>
		<td<?php echo $frl_celula->frl_cel_id->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_frl_cel_id" class="frl_celula_frl_cel_id">
...
<?php } ?>
<?php if ($frl_celula->celula->Visible) { // celula ?>
		<td<?php echo $frl_celula->celula->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_celula" class="frl_celula_celula">
...
<?php } ?>
<?php if ($frl_celula->cod_provincia->Visible) { // cod_provincia ?>
		<td<?php echo $frl_celula->cod_provincia->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_cod_provincia" class="frl_celula_cod_provincia">
...
<?php } ?>
<?php if ($frl_celula->cod_distrito->Visible) { // cod_distrito ?>
		<td<?php echo $frl_celula->cod_distrito->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_cod_distrito" class="frl_celula_cod_distrito">
...
<?php } ?>
<?php if ($frl_celula->cod_zona->Visible) { // cod_zona ?>
		<td<?php echo $frl_celula->cod_zona->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_cod_zona" class="frl_celula_cod_zona">
...
<?php } ?>
<?php if ($frl_celula->cod_circulo->Visible) { // cod_circulo ?>
		<td<?php echo $frl_celula->cod_circulo->CellAttributes() ?>>
<span id="el<?php echo $frl_celula_delete->RowCnt ?>_frl_celula_cod_circulo" class="frl_celula_cod_circulo">
...
<?php
	$frl_celula_delete->Recordset->MoveNext();
}
$frl_celula_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $frl_celula_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ffrl_celuladelete.Init();
</script>
<?php
$frl_celula_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$frl_celula_delete->Page_Terminate();
?>
